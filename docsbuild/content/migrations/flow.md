...
| buildIn        | Boolean                               | Yes       | false         |
| providerId     | String                                | Yes       | "basic-flow"  |
| topLevel       | Boolean                               | Yes       | true          |
| executions     | List< AuthenticationExecutionImport > | Yes       | emptyList()   |
...
| providerId     | String              | No        | _see providers [here](#known-flow-providers-on-release-09042021)_                                            |               |
| level          | Int                 | No        |                                                                                                              |               |
| index          | Int                 | No        |                                                                                                              |               |
...
| basic-auth-otp|Basic Auth Password+OTP|Challenge-response authentication using HTTP BASIC scheme. Password param should contain a combination of password + otp. Realm's OTP policy is used to determine how to parse this. This SHOULD NOT BE USED in conjection with regular basic auth provider. || idp-review-profile|Review Profile|User reviews and updates profile data retrieved from Identity Provider in the displayed form |
